...
class MMSEPredictor:
    ...
    def predict(self, audio_data, transcript_text=None):
        ...
        with torch.no_grad():
            logits = self.model(pixels_tensor, input_ids, attn_mask)
        ...
        return {
            ...
            'mmse_probabilities': {lbl: round(p.item(), 4) for lbl, p in zip(MMSE_LABEL_MAP.values(), probs)}
        }
