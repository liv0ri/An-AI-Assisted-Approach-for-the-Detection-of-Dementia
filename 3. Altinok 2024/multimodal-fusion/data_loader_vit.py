...
def parse_mimse(scores_file): 
  ...
  with open(scores_file, newline='') as csvfile: 
    ...
    next(reader) 
    for row in reader:
      mdict[row[1]] = int(row[2]) # Assigns the integer value from the third column as the MMSE score to the key from the second column.
  return mdict 
...
class AdressoDataset(Dataset): 
    def __init__(self, phase): 
      if phase == "train": 
        ...
        self.all_filen, self.specto_filen, self.trans_filen, self.labels, self.mimse_scores = shuffle(all_filen, specto_filen, trans_filen, labels, mimse_scores, random_state=44) # Shuffles all corresponding lists (filenames, spectrogram paths, transcription paths, labels, MMSE scores) consistently using a fixed random_state for reproducibility.
        ...
        mimse_dict = parse_mimse_test(mimse_file) # Parses the test MMSE scores CSV file.
        ...
        self.all_filen, self.specto_filen, self.trans_filen, self.labels, self.mimse_scores = all_filen, specto_files, trans_files, labels, mimse_scores # Assigns the prepared lists to instance variables. No shuffling for the test set.
