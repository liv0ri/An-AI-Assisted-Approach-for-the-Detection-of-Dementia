...
def train_and_save_audio_text_model(dataset_type, remove_short_sentences):
    ...
    if not os.path.exists(VOCAB_PATH) or not os.path.exists(WORD2VEC_PATH):
        raise FileNotFoundError("Vocab or Word2Vec vectors not found. Please run the build_vocab_and_vectors script first.")
    ...
    for train_val_index, test_index in kf.split(all_audios, all_labels):
        ...
        model.fit([audio_train, word_train_padded], y_train,
                  ...
                  callbacks=callback,
                  verbose = 0)
        fold_model_path = os.path.join(model_save_dir, f"audio_text_model_fold_{fold_number}.keras")
    ...
    auc_results = eval_results_array[:, 4]  
...
if __name__ == "__main__":
    ...
    train_and_save_audio_text_model(dataset_type="augmented", remove_short_sentences=False)
