...
def train_and_save_multimodal_model(dataset_type, remove_short_sentences):
    ...
    if not os.path.exists(VOCAB_PATH) or not os.path.exists(WORD2VEC_PATH):
        raise FileNotFoundError("Vocab or Word2Vec vectors not found. Please run build_vocab_and_vectors first.")
    ...
    for train_val_index, test_index in kf.split(all_word_ids, all_labels):
        ...
        fold_model_path = os.path.join(model_save_dir, f"audio_text_time_model_fold_{fold_number}.keras")
...
if __name__ == "__main__":
    ...
    train_and_save_multimodal_model(dataset_type="augmented", remove_short_sentences=False)